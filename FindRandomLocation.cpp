...
EBTNodeResult::Type UFindRandomLocation::ExecuteTask(UBehaviorTreeComponent& owner_comp, uint8* node_memory)
{
	...
	auto const controller = Cast<ANPC_AIController>(owner_comp.GetAIOwner());
	...
	if (nav_sys->GetRandomPointInNavigableRadius(origin, f_SearchRadius, location, nullptr))
	{

		controller->get_blackboard()->SetValueAsVector(GetSelectedBlackboardKey(), location.Location);
	}
